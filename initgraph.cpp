...
//输入函数
char *text_input(int y,int choice);//y参数是文字显示的纵坐标,choice参数是年月的选择,1为年份,2为月份.
void advance()
{
    initgraph();

    year =  text_input(0,1);
    month = text_input(100,2);
}
void main_1()
{
...
char *text_input(int y,int choice) {
    SDL_Event event;
    char *text_0;
    char *text_1;
    char *editing;
    ...
    while (1) {
        if (x == 0 && choice == 1)
        {
            draw_font("请输入要查询的年份:",0,0);
            SDL_RenderPresent(rdr);
            strcpy(text,"");
        }
        if (x == 0 && choice == 2)
        {
            draw_font("请输入要查询的月份:",0,100);
            SDL_RenderPresent(rdr);
            strcpy(text,"");
        }
